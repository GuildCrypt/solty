...
export class SoltyUint112 extends Solty {
  readonly bytesLength = 14
  static bytesLength = 14

  constructor(bytes: Uint8Array) {
    super(bytes)
    if (bytes.length !== this.bytesLength) {
      throw new InvalidBytesLengthError('Uint112', this.bytesLength, bytes.length)
    }
  }

  static getRandom(hexu: string): SoltyUint112 {
    return new this(getRandomUint8Array(this.bytesLength))
  }
  static fromHexu(hexu: string): SoltyUint112 {
    return new this(convertHexuToUint8Array(hexu))
  }
  static fromHexp(hexp: string): SoltyUint112 {
