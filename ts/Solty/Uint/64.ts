...
export class SoltyUint64 extends Solty {
  readonly bytesLength = 8
  static bytesLength = 8

  constructor(bytes: Uint8Array) {
    super(bytes)
    if (bytes.length !== this.bytesLength) {
      throw new InvalidBytesLengthError('Uint64', this.bytesLength, bytes.length)
    }
  }

  static getRandom(hexu: string): SoltyUint64 {
    return new this(getRandomUint8Array(this.bytesLength))
  }
  static fromHexu(hexu: string): SoltyUint64 {
    return new this(convertHexuToUint8Array(hexu))
  }
  static fromHexp(hexp: string): SoltyUint64 {
