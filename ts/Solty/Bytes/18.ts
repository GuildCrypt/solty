...
export class SoltyBytes18 extends Solty {
  readonly bytesLength = 18
  static bytesLength = 18

  constructor(bytes: Uint8Array) {
    super(bytes)
    if (bytes.length !== this.bytesLength) {
      throw new InvalidBytesLengthError('Bytes18', this.bytesLength, bytes.length)
    }
  }

  static getRandom(hexu: string): SoltyBytes18 {
    return new this(getRandomUint8Array(this.bytesLength))
  }
  static fromHexu(hexu: string): SoltyBytes18 {
    return new this(convertHexuToUint8Array(hexu))
  }
  static fromHexp(hexp: string): SoltyBytes18 {
